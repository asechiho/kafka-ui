...
func (rethinkService *RethinkDBService) config(configure *config.Configure) {
	rethinkService.configure = configure
}

func (rethinkService *RethinkDBService) Topics(socketContext context.Context, startChan <-chan interface{}) <-chan Message {
	var (
...
func (rethinkService *RethinkDBService) Messages(socketContext context.Context, filterChan <-chan Filters) <-chan Message {
	msgChan := make(chan Message, 1)
...
func (rethinkService *RethinkDBService) listenChanges(socketContext context.Context, id uuid.UUID, msgChan chan Message) {
	go func() {
		var (
			changes Changes
			cursor  *rethink.Cursor
...
func (rethinkService *RethinkDBService) Serve() {
	var (
		err        error
		retryCount = 10
	)
...
func (rethinkService *RethinkDBService) Stop() {
}

func (rethinkService *RethinkDBService) InitializeContext() error {
	var (
		err error
		id  uuid.UUID
	)
...
func (rethinkService *RethinkDBService) connect(isDbCreated bool) (uuid.UUID, error) {
	var (
...
func (rethinkService *RethinkDBService) close(id uuid.UUID) {
	_ = rethinkService.getConnection(id).Close()
	rethinkService.mutex.Lock()
	delete(rethinkService.connectionPool, id)
	rethinkService.mutex.Unlock()
}

func (rethinkService *RethinkDBService) executeCreateIfAbsent(listTerm rethink.Term, createTerm rethink.Term, id uuid.UUID) error {
	var (
		isContains bool
...
func (rethinkService *RethinkDBService) getLastMessages(id uuid.UUID, msgChan chan Message, filters Filters, count int) {
...
func (rethinkService *RethinkDBService) appendTopic(topic string) {
...
func (rethinkService *RethinkDBService) getConnection(id uuid.UUID) *rethink.Session {
...
func (rethinkService *RethinkDBService) createConnection(session *rethink.Session) uuid.UUID {
	id := uuid.New()
	rethinkService.mutex.Lock()
